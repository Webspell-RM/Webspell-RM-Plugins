...
global $userID;
    $moduls = array();
    $moduls['vi'] = array("videos","videosID","comments");
    $allowed = 0;
    $modul = $moduls[$type];
...
if (isset($_POST[ 'savevisitorcomment' ])) {
   ...
   if (!empty($name) && !empty($message)) {
        $date = time();
        $ip = $GLOBALS[ 'ip' ];
        $ergebnis = safe_query("SELECT * FROM " . PREFIX . "plugins_videos_comments ORDER BY date DESC LIMIT 0,1");
        ...
        safe_query(
                "INSERT INTO `" . PREFIX . "plugins_videos_comments` (
                    `parentID`,
                    `type`,
                    `nickname`,
                    `date`,
...
$name = getnickname($userID, $id);
    ...
    $msgfrom= array();
    $msgfrom[ 'vi' ] = ''.$plugin_language[ 'to_an_video' ].'';
...
//ende /////////////////////////////////////////////////////////////////////     
    ...
    foreach ($_POST[ 'commentID' ] as $id) {
        safe_query("DELETE FROM " . PREFIX . "plugins_videos_comments WHERE commentID='" . (int)$id."'");
    }
    header("Location: " . $_POST[ 'referer' ]);
} else {    
    ...
    if ($gesamt) {
        ...
        while ($ds = mysqli_fetch_array($ergebnis)) {
            ...
            if (isfeedbackadmin($userID) || iscommentposter($userID, $ds[ 'commentID' ])) {
                $edit = '<a class="badge text-bg-warning" href="index.php?site=videos&action=editcomment&id=' . $ds[ 'commentID' ] . '&amp;ref=' .
            ...
            $avatar = '<img style="height:45px" class="rounded-circle" src="images/avatars/' . getavatar($ds[ 'userID' ]) . '" class="text-left" alt="Avatar">'; 
