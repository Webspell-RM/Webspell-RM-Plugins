...
global $userID;
    $moduls = array();
    $moduls['po'] = array("polls","pollID","comments");
    $allowed = 0;
    $modul = $moduls[$type];
...
if (isset($_POST[ 'savevisitorcomment' ])) {
   ...
   if (!empty($name) && !empty($message)) {
        $date = time();
        $ip = $GLOBALS[ 'ip' ];
        $ergebnis = safe_query("SELECT * FROM " . PREFIX . "plugins_polls_comments ORDER BY date DESC LIMIT 0,1");
        ...
        safe_query(
                "INSERT INTO `" . PREFIX . "plugins_polls_comments` (
                    `parentID`,
                    `type`,
                    `nickname`,
                    `date`,
...
$name = getnickname($userID, $id);
    ...
    $msgfrom= array();
    $msgfrom[ 'po' ] = ''.$plugin_language[ 'to_an_poll' ].'';
...
//ende /////////////////////////////////////////////////////////////////////     
    ...
    foreach ($_POST[ 'commentID' ] as $id) {
        safe_query("DELETE FROM " . PREFIX . "plugins_polls_comments WHERE commentID='" . (int)$id."'");
    }
    header("Location: " . $_POST[ 'referer' ]);
} else {
    ...
    if ($gesamt) {
        ...
        while ($ds = mysqli_fetch_array($ergebnis)) {
            ...
            if (isfeedbackadmin($userID) || iscommentposter($userID, $ds[ 'commentID' ])) {
                $edit = '<a class="badge text-bg-warning" href="index.php?site=polls&action=editcomment&id=' . $ds[ 'commentID' ] . '&amp;ref=' .
