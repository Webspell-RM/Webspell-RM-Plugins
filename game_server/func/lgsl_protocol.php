...
// ^7 = white
  ...
  function lgsl_parse_color($string, $type)
  {
    ...
    while(preg_match('/\^([0-9])/', $string)) {
      foreach(array('black', 'red', 'darkgreen', 'yellow', 'blue', 'cyan', 'pink', 'rednight', 'bluenight', 'white') as $num_color => $name_color) {
  if (preg_match('/\^([0-9])(.*)\^([0-9])/', $string)) {
    $string = preg_replace("#\^".$num_color."(.*)\^([0-9])#Usi", "<span class=\"gsquery-".$name_color."\">$1 ^$2", $string);
  } else {
    $string = preg_replace("#\^".$num_color."(.*)$#Usi", "<span class=\"gsquery-".$name_color."\">$1", $string);
  }
      }
    }
    #return $string;
    break;
        ...
        #$string = preg_replace("#\^".$num_color."(.*)\^([0-9])#Usi", "<span class=\"gsquery-".$name_color."\">$1</span>^$2", $string);
