...
if (isset($_POST['submit'])) {
    ...
    if ($screen->hasFile()) {
        if ($screen->hasError() === false) {
            ...
            if ($screen->saveAs($new_name)) {
                ...
                $ergebnis = safe_query("SELECT screens FROM " . PREFIX . "plugins_news_manager WHERE newsID='" . $_POST["newsID"] . "'");
    ...
    if ($CAPCLASS->checkCaptcha(0, $_POST['captcha_hash'])) {
                ...
                if ($upload->supportedMimeType($mime_types)) {
                    ...
                    if (is_array($imageInformation)) {
                        ...
                        if ($upload->saveAs($filepath . $file, true)) {
                            ...
                            safe_query(
                                "UPDATE " . PREFIX . "plugins_news_manager SET screens='" . $file . "' WHERE newsID='" . $id . "'"
                            );
                        }
                    } else {
        ...
        $rubrics = '';
        $newsrubrics = safe_query("SELECT rubricID, rubric FROM " . PREFIX . "plugins_news_manager_rubrics ORDER BY rubric");
...
if (isset($_POST['news_categories_save'])) {
    ...
    if ($CAPCLASS->checkCaptcha(0, $_POST['captcha_hash'])) {
        if (checkforempty(array('name'))) {
            safe_query("INSERT INTO " . PREFIX . "plugins_news_manager_rubrics ( rubric ) values( '" . $_POST['name'] . "' ) ");
            $id = mysqli_insert_id($_database);
                    ...
                    if ($upload->supportedMimeType($mime_types)) {
                        ...
                        if (is_array($imageInformation)) {
                            ...
                            if ($upload->saveAs($filepath . $file, true)) {
                                ...
                                safe_query(
                                    "UPDATE " . PREFIX . "plugins_news_manager_rubrics
    ...
    if ($CAPCLASS->checkCaptcha(0, $_POST['captcha_hash'])) {
        if (checkforempty(array('name'))) {
            safe_query(
                "UPDATE
                    `" . PREFIX . "plugins_news_manager_rubrics`
                SET
                    `rubric` = '" . $_POST['name'] . "'
                WHERE
    ...
    if ($CAPCLASS->checkCaptcha(0, $_POST['captcha_hash'])) {
        safe_query(
            "UPDATE
                " . PREFIX . "plugins_news_manager_settings
            SET
    ...
    if ($CAPCLASS->checkCaptcha(0, $_GET['captcha_hash'])) {
        ...
        $newsID = $_GET['newsID'];
        $dg = mysqli_fetch_array(safe_query("SELECT * FROM " . PREFIX . "plugins_news_manager WHERE newsID='" . $_GET['newsID'] . "'"));
        $screens = array();
        if (!empty($dg['screens'])) {
        ...
        safe_query("DELETE FROM " . PREFIX . "plugins_news_manager WHERE newsID='" . $_GET['newsID'] . "'");
    ...
    if ($CAPCLASS->checkCaptcha(0, $_GET['captcha_hash'])) {
        ...
        safe_query("DELETE FROM " . PREFIX . "plugins_news_manager_rubrics WHERE rubricID='$rubricID'");
...
if ($action == "add") {
    ...
    $ergebnis = safe_query("SELECT * FROM `" . PREFIX . "plugins_news_manager`");
    $ds = mysqli_fetch_array($ergebnis);

    $rubriccategory = safe_query("SELECT * FROM `" . PREFIX . "plugins_news_manager_rubrics` ORDER BY `rubric`");
...
<div class="mb-3 row">
  ...
  <div class="mb-3 row">
    ...
    $ergebnis = safe_query("SELECT * FROM `" . PREFIX . "plugins_news_manager` WHERE `newsID` = '$newsID'");
    $ds = mysqli_fetch_array($ergebnis);
...
';
    ...
    $ergebnis = safe_query("SELECT screens FROM " . PREFIX . "plugins_news_manager WHERE newsID='" . $newsID . "'");
...
<div class="mb-3 row">
    ...
    <div class="col-sm-3">
        <input class="hidden-xs hidden-sm btn btn-danger" type="button" onclick="confirmDelete(\'' . $plugin_language['delete'] . '\', \'admincenter.php?site=admin_news_manager&action=picdelete&newsID=' . $newsID . '&file=' . basename($screen) . '\')" value="' . $plugin_language['delete'] . '">                    
    </div>
</div>
<hr>
...
<div class="mb-3 row">
  ...
  <div class="mb-3 row">
    ...
    $ergebnis = safe_query("SELECT screens FROM " . PREFIX . "plugins_news_manager WHERE newsID=" . $_GET["newsID"] . "");
    $db = mysqli_fetch_array($ergebnis);
    ...
    safe_query("UPDATE " . PREFIX . "plugins_news_manager SET screens='" . $newscreens_string . "' WHERE newsID=" . $_GET["newsID"] . "");
  ...
  <div class="card-header">
    ...
    $alle = safe_query("SELECT newsID FROM " . PREFIX . "plugins_news_manager");
    ...
    if ($page == "1") {
        $ergebnis = safe_query("SELECT * FROM " . PREFIX . "plugins_news_manager ORDER BY date DESC LIMIT 0,$max");
        $n = 1;
    } else {
        $start = $page * $max - $max;
        $ergebnis = safe_query("SELECT * FROM " . PREFIX . "plugins_news_manager ORDER BY date DESC LIMIT $start,$max");
...
<button class="btn btn-primary" type="submit" name="news_settings_save">' . $plugin_language['update'] . '</button>
  ...
  <div class="card-header">
    ...
    $ds = mysqli_fetch_array(
        safe_query(
            "SELECT * FROM " . PREFIX . "plugins_news_manager_rubrics WHERE rubricID='" . intval($_GET['rubricID']) . "'"
        )
    );
...
<div class="col-md-12"><br />';
    ...
    $ergebnis = safe_query("SELECT * FROM " . PREFIX . "plugins_news_manager_rubrics ORDER BY rubric");
    $filepath = $plugin_path . "images/news-rubrics/";
    echo '<table class="table table-striped">
    <thead>
      <tr>
