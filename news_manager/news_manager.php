...
# Sprachdateien aus dem Plugin-Ordner laden
    ...
    $alle=safe_query("SELECT newsID FROM ".PREFIX."plugins_news WHERE displayed = '1'");
  ...
  if ($page == "1") {
   ...
   if($pages>1) echo $page_link;
    ...
    if (mysqli_num_rows($ergebnis)) {    
        ...
        $headlines = '<a href="index.php?site=news_manager&action=news_contents&amp;newsID=' . $ds[ 'newsID' ] . '">' .
                    $ds['headline'] . '</a><br>';
         
        $content = $ds['content'];
        $max_content = $ds[ 'content' ];
        ...
        $data_array['$content'] = $content;
        $data_array['$max_content'] = $max_content;
