...
#-----------------------
	...
	if($_recom=="rem" and isset($_GET['rid'])) { 
		if(isanyadmin($userID) and is_numeric($_GET['rid'])) {
			try {
				safe_query("DELETE FROM `".PREFIX."plugins_news_comments_recomment` WHERE `recoID` = '".$_GET['rid']."' LIMIT 1;");
	...
	if(isset($_POST['saverecomment'])){
		if(!$userID) { echo "Access Denied!"; echo '<script>window.history.back();</script>'; return false; }
				try {
					safe_query("INSERT INTO `".PREFIX."plugins_news_comments_recomment` (`comment_id`, `user_id`, `datetime`, `comment`, `type`, `parentID` ) VALUES ('".$_POST['comID']."', '".$_POST['userID']."', '".time()."', '".$_POST['message2']."', '".$_POST['type']."', '".$_POST['parentID']."');");
				} Catch(Exception $e) {
					//errs
				}
		echo '<script>window.history.back();</script>';
