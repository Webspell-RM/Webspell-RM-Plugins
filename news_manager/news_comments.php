...
if ($_recom == "rem" and isset($_GET['rid'])) {
    if (isanyadmin($userID) and is_numeric($_GET['rid'])) {
        try {
            safe_query("DELETE FROM `" . PREFIX . "plugins_news_manager_comments_recomment` WHERE `recoID` = '" . $_GET['rid'] . "' LIMIT 1;");
...
if (isset($_POST['saverecomment'])) {
    ...
    try {
        safe_query("INSERT INTO `" . PREFIX . "plugins_news_manager_comments_recomment` (`comment_id`, `user_id`, `datetime`, `comment`, `type`, `parentID` ) VALUES ('" . $_POST['comID'] . "', '" . $_POST['userID'] . "', '" . time() . "', '" . $_POST['message2'] . "', '" . $_POST['type'] . "', '" . $_POST['parentID'] . "');");
    } catch (Exception $e) {
        //errs
    }
    echo '<script>window.history.back();</script>';
...
if (!empty($name) && !empty($message)) {
    $date = time();
    $ip = $GLOBALS['ip'];
    $ergebnis = safe_query("SELECT * FROM " . PREFIX . "plugins_news_manager_comments ORDER BY date DESC LIMIT 0,1");
    ...
    if (($ds['message'] != $message) ||
        ...
        safe_query(
            "INSERT INTO `" . PREFIX . "plugins_news_manager_comments` (
                    `parentID`,
                    `type`,
                    `nickname`,
                    `date`,
    ...
    foreach ($_POST['commentID'] as $id) {
        safe_query("DELETE FROM " . PREFIX . "plugins_news_manager_comments WHERE commentID='" . (int)$id . "'");
    }
    ...
    if ($gesamt) {
        ...
        while ($ds = mysqli_fetch_array($ergebnis)) {
            ...
            if ($ds['userID']) {
                ...
                if (
                    $gethomepage != "" && $gethomepage != "http://" && $gethomepage != "http:///"
                    && $gethomepage != "n/a"
                ) {
            ...
            if ($userID) {
                $recomm = '<a class="badge text-bg-success btn-sm" title="' . $plugin_language['comment_reply'] . '" href="javascript: void(0)" onclick="javascript: document.getElementById(\'rec' . $cid . '\').style.display=\'inline\'"><span> ' . $plugin_language['comment_reply'] . '</span></a>';
            } else {
                $recomm = "";
            }
