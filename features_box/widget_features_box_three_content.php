...
$plugin_language = $pm->plugin_language("features_box", $plugin_path);
	...
	$data_array = array();
	$template = $GLOBALS["_template"]->loadTemplate("features_three_box","head", $data_array, $plugin_path);
	echo $template;
...
if (mysqli_num_rows($ergebnis)) {
	...
	$template = $GLOBALS["_template"]->loadTemplate("features_three_box","content_about", $data_array, $plugin_path);
	echo $template;
} else {
	$data_array = array();
	$data_array['$no_about'] = $plugin_language['no_about'];
    $template = $GLOBALS["_template"]->loadTemplate("features_three_box","content_about_no", $data_array, $plugin_path);
	echo $template;
}
#################################
...
if (mysqli_num_rows($ergebnis)) {
  ...
  <iframe src="https://www.youtube.com/embed/'.$videoID.'?rel=0" title="YouTube video" allowfullscreen style="border-radius: var(--bs-border-radius);"></iframe>
		...
		$template = $GLOBALS["_template"]->loadTemplate("features_three_box","content_video", $data_array, $plugin_path);
		echo $template;
	}
} else {
	$data_array = array();
	$data_array['$no_videos'] = $plugin_language['no_videos'];
    $template = $GLOBALS["_template"]->loadTemplate("features_three_box","content_video_no", $data_array, $plugin_path);
	echo $template;
}
...
if (mysqli_num_rows($ergebnis)) {
	...
	$ergebnis = safe_query("SELECT picID, name, views FROM " . PREFIX . "plugins_gallery_pictures WHERE (picID <> '') ORDER BY RAND() LIMIT 1");
		...
		if (mysqli_num_rows($ergebnis)) {
			...
			while ($dx = mysqli_fetch_array($ergebnis)) {
				...
				$template = $GLOBALS["_template"]->loadTemplate("features_three_box","content_gallery", $data_array, $plugin_path);
				echo $template;
			}
		}
	}
    ...
    $template = $GLOBALS["_template"]->loadTemplate("features_three_box","content_gallery_no", $data_array, $plugin_path);
	echo $template;
}
	$data_array = array();
	$template = $GLOBALS["_template"]->loadTemplate("features_three_box","foot", $data_array, $plugin_path);
	echo $template;
