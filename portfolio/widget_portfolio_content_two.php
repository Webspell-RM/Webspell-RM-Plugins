...
if (isset($_GET[ 'action' ])) {
    ...
    $template = $GLOBALS["_template"]->loadTemplate("portfolio_widget","title_head", $data_array, $plugin_path);
    echo $template;
...
if ($action == "show" && is_numeric($_GET[ 'portfoliocatID' ])) {
    ...
    if (mysqli_num_rows($cats)) {
        ...
        $template = $GLOBALS["_template"]->loadTemplate("portfolio_widget","category", $data_array, $plugin_path);
        echo $template;
        ...
        while ($ds = mysqli_fetch_array($cats)) {
            ...
            $template = $GLOBALS["_template"]->loadTemplate("portfolio_widget","content", $data_array, $plugin_path);
            echo $template;

        }

        $template = $GLOBALS["_template"]->loadTemplate("portfolio_widget","foot", $data_array, $plugin_path);
        echo $template;

        $data_array= array();
        
        $template = $GLOBALS["_template"]->loadTemplate("portfolio_widget","cat_all", $data_array, $plugin_path);
        echo $template;

        $template = $GLOBALS["_template"]->loadTemplate("portfolio_widget","cat_head", $data_array, $plugin_path);
            echo $template;
        ...
        while($ds=mysqli_fetch_array($ergebnis)) {
            ...
            $query = safe_query("SELECT portfolioID FROM " . PREFIX . "plugins_portfolio");
            ...
            $template = $GLOBALS["_template"]->loadTemplate("portfolio_widget","cat", $data_array, $plugin_path);
            echo $template;

             $n++;
        }
            $template = $GLOBALS["_template"]->loadTemplate("portfolio_widget","cat_foot", $data_array, $plugin_path);
            echo $template;

    } else {        
        echo $_lang[ 'no_categories' ];
        $template = $GLOBALS["_template"]->loadTemplate("portfolio_widget","cat_foot_no", $data_array, $plugin_path);
        echo $template;
    }
}
?>
