...
$plugin_language = $pm->plugin_language("navigation_sticky", $plugin_path);

$ergebnis = safe_query("SELECT * FROM ".PREFIX."navigation_dashboard_links WHERE modulname='navigation_sticky'");
...
if ($action == "add") {
    ...
    <ol class="breadcrumb">
        <li class="breadcrumb-item active" aria-current="page"><a href="admincenter.php?site=admin_navigation_sticky" class="white">' . $plugin_language[ 'title' ] . '</a></li>
    ...
    echo'<form class="form-horizontal" method="post" action="admincenter.php?site=admin_navigation_sticky" enctype="multipart/form-data">
   <div class="mb-3 row">
...
<div class="card-body">';
    ...
    $ds = mysqli_fetch_array(
        safe_query(
            "SELECT * FROM " . PREFIX . "plugins_navigation_sticky WHERE headerID='" . intval($_GET['headerID']) ."'"
        )
    );
...
<div class="mb-3 row">
    <label class="col-sm-2 control-label">' . $plugin_language[ 'head_text' ] . ':</label>
    <div class="col-sm-8"><span class="text-muted small"><em>
        <input class="form-control" type="text" name="head_text" value="' . $head_text . '" size="97" /></em></span>
    </div>
  </div>
...
<div class="mb-3 row">
    ...
    <div class="col-sm-8"><span class="text-muted small"><em>
        <input class="form-control" type="text" name="text" value="' . $text . '" size="97" /></em></span>
    </div>
  </div>
...
<div class="mb-3 row">
    ...
    <div class="col-sm-8"><span class="text-muted small"><em>
        <input class="form-control" type="text" name="link" value="' . $link . '" size="97" /></em></span>
    </div>
  </div>
...
<div class="form-group">
    ...
    safe_query("INSERT INTO `".PREFIX."plugins_navigation_sticky` (head_text, text, link) values ('$head_text','$text','$link')");
                ...
                if ($upload->supportedMimeType($mime_types)) {
                    ...
                    if (is_array($imageInformation)) {
                        ...
                        if ($upload->saveAs($filepath.$file, true)) {
                            ...
                            safe_query(
                                "UPDATE " . PREFIX . "plugins_navigation_sticky SET header_pic='" . $file . "' WHERE headerID='" . $id . "'"
                            );
                        }
                    } else {
        ...
        if (count($errors)) {
            ...
            redirect("admincenter.php?site=admin_navigation_sticky", "", 0);
        }
    } else {
    ...
    if ($CAPCLASS->checkCaptcha(0, $_POST[ 'captcha_hash' ])) {
        ...
        safe_query(
            "UPDATE " . PREFIX . "plugins_navigation_sticky 
            SET 
            head_text = '" . $head_text . "',
            text = '" . $text . "',
            link = '" . $link . "'
            WHERE headerID='" . $_POST[ "headerID" ] . "'");
    ...
    if ($CAPCLASS->checkCaptcha(0, $_GET[ 'captcha_hash' ])) {
        $get = safe_query("SELECT * FROM " . PREFIX . "plugins_navigation_sticky WHERE headerID='" . $_GET[ "headerID" ] . "'");
        $data = mysqli_fetch_assoc($get);
 
        if (safe_query("DELETE FROM " . PREFIX . "plugins_navigation_sticky WHERE headerID='" . $_GET[ "headerID" ] . "'")) {
...
if (isset($ds['headerID']) ? isset($ds['headerID']) : 0) {  
    ...
    <div class="col-md-8"><a href="admincenter.php?site=admin_navigation_sticky&amp;action=add" class="btn btn-primary" type="button">' . $plugin_language[ 'new_header' ] . '</a></div>';
}
      echo' '.$add.'
    
  </div>';
 
    echo '<form method="post" action="admincenter.php?site=admin_navigation_sticky">
   ...
   $CAPCLASS = new \webspell\Captcha;
    ...
    $qry = safe_query("SELECT * FROM " . PREFIX . "plugins_navigation_sticky");
    ...
    if ($anz) {
        ...
        while ($ds = mysqli_fetch_array($qry)) {
            ...
            echo '<tr>
           <td class="' . $td . ' col-5"><img class="img-thumbnail" style="height: 200px" align="center" src="../' . $filepath . $ds[ 'header_pic' ] . '" alt="{img}" /></td>
           <!--<td class="' . $td . ' col-5">
            <div class="mb-3 row">
                <div class="col-auto">
           ...
           <td class="' . $td . ' col-2"><a href="admincenter.php?site=admin_navigation_sticky&amp;action=edit&amp;headerID=' . $ds[ 'headerID' ] .
