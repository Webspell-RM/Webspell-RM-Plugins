...
$ergebnis = safe_query("SELECT * FROM ".PREFIX."navigation_dashboard_links WHERE modulname='projectslider'");
...
if ($action == "add") {
  ...
  $ds = mysqli_fetch_array(
        safe_query(
            "SELECT * FROM " . PREFIX . "plugins_projectslider"
        )
    );
 ...
 echo '<div class="card">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="admincenter.php?site=admin_projectslider">' . $plugin_language[ 'carousel' ] . '</a></li>
    ...
    echo'<form class="form-horizontal" method="post" action="admincenter.php?site=admin_projectslider" enctype="multipart/form-data">
...
echo '<div class="card">
    ...
    $ds = mysqli_fetch_array(
        safe_query(
            "SELECT * FROM " . PREFIX . "plugins_projectslider WHERE carouselID='" . intval($_GET['carouselID']) ."'"
        )
    );
...
<div class="form-group">
    ...
    if ($CAPCLASS->checkCaptcha(0, $_POST[ 'captcha_hash' ])) {
        ...
        if (is_array($sort)) {
            foreach ($sort as $sortstring) {
                ...
                safe_query("UPDATE " . PREFIX . "plugins_projectslider SET sort='$sorter[1]' WHERE carouselID='$sorter[0]' ");
                redirect("admincenter.php?site=admin_projectslider", "", 0);
            }
        }
    } else {
    ...
    $CAPCLASS = new \webspell\Captcha;
    if ($CAPCLASS->checkCaptcha(0, $_POST[ 'captcha_hash' ])) {
    safe_query("INSERT INTO `".PREFIX."plugins_projectslider` (title, ani_title, link, ani_link, description, ani_description, displayed, sort) values ('".$title."','".$ani_title."', '".$link."', '".$ani_link."', '".$description."', '".$ani_description."', '".intval($displayed)."','1')");
                ...
                if ($upload->supportedMimeType($mime_types)) {
                    ...
                    if (is_array($imageInformation)) {
                        ...
                        if ($upload->saveAs($filepath.$file, true)) {
                            ...
                            safe_query(
                                "UPDATE " . PREFIX . "plugins_projectslider SET carousel_pic='" . $file . "' WHERE carouselID='" . $id . "'"
                            );
                        }
                    } else {
    ...
    if ($CAPCLASS->checkCaptcha(0, $_POST[ 'captcha_hash' ])) {
        ...
        safe_query(
            "UPDATE " . PREFIX . "plugins_projectslider SET title='" . $title . "', ani_title='" . $ani_title . "', link='" . $link . "', ani_link='" . $ani_link . "', description='" . $description .
    ...
    if ($CAPCLASS->checkCaptcha(0, $_GET[ 'captcha_hash' ])) {
        $get = safe_query("SELECT * FROM " . PREFIX . "plugins_projectslider WHERE carouselID='" . $_GET[ "carouselID" ] . "'");
        $data = mysqli_fetch_assoc($get);
 
        if (safe_query("DELETE FROM " . PREFIX . "plugins_projectslider WHERE carouselID='" . $_GET[ "carouselID" ] . "'")) {
...
<div class="form-group row">
    ...
    <div class="col-md-8">
      <a href="admincenter.php?site=admin_projectslider&amp;action=add" class="btn btn-primary" type="button">' . $plugin_language[ 'new_carousel' ] . '</a>
    </div>
  </div>';

 
    echo '<form method="post" action="admincenter.php?site=admin_projectslider">
   ...
   $CAPCLASS = new \webspell\Captcha;
    ...
    $qry = safe_query("SELECT * FROM " . PREFIX . "plugins_projectslider ORDER BY sort");
    ...
    if ($anz) {
        ...
        while ($ds = mysqli_fetch_array($qry)) {
           ...
           <td class="' . $td . '"><img class="img-thumbnail" style="width: 100%; max-width: 350px" align="center" src="../' . $filepath . $ds[ 'carousel_pic' ] . '" alt="{img}" /></td>
           <td class="' . $td . '">' . $displayed . '</td>
           <td class="' . $td . '"><a href="admincenter.php?site=admin_projectslider&amp;action=edit&amp;carouselID=' . $ds[ 'carouselID' ] .
                '" class="btn btn-warning" type="button">' . $plugin_language[ 'edit' ] . '</a>

        <!-- Button trigger modal -->
    <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#confirm-delete" data-href="admincenter.php?site=admin_projectslider&amp;delete=true&amp;carouselID=' . $ds[ 'carouselID' ] . '&amp;captcha_hash=' . $hash . '">
