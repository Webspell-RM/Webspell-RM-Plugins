...
global $userID;
    $moduls = array();
    $moduls['ar'] = array("articles","articleID","comments");
    $allowed = 0;
    $modul = $moduls[$type];
...
if (isset($_POST[ 'savevisitorcomment' ])) {
   ...
   if (!empty($name) && !empty($message)) {
        $date = time();
        $ip = $GLOBALS[ 'ip' ];
        $ergebnis = safe_query("SELECT * FROM " . PREFIX . "plugins_articles_comments ORDER BY date DESC LIMIT 0,1");
        ...
        safe_query(
                "INSERT INTO `" . PREFIX . "plugins_articles_comments` (
                    `parentID`,
                    `type`,
                    `nickname`,
                    `date`,
...
$name = getnickname($userID, $id);
    ...
    $msgfrom= array();
    $msgfrom[ 'ar' ] = ''.$plugin_language[ 'to_an_article' ].'';
...
//ende /////////////////////////////////////////////////////////////////////     
    ...
    foreach ($_POST[ 'commentID' ] as $id) {
        safe_query("DELETE FROM " . PREFIX . "plugins_articles_comments WHERE commentID='" . (int)$id."'");
    }
    header("Location: " . $_POST[ 'referer' ]);
    ...
    if ($gesamt) {
        ...
        while ($ds = mysqli_fetch_array($ergebnis)) {
            ...
            if (isfeedbackadmin($userID) || iscommentposter($userID, $ds[ 'commentID' ])) {
                $edit = '<a class="badge text-bg-warning" href="index.php?site=articles&action=editcomment&id=' . $ds[ 'commentID' ] . '&amp;ref=' .
