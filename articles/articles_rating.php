...
if (isset($table) && isset($key)) {
    ...
    if (mysqli_num_rows($getarticles)) {
        ...
        if ($go === true) {
            ...
            $ergebnis = safe_query(
                "SELECT votes, points FROM " . PREFIX . "plugins_articles WHERE articleID = '" . (int)$id."'"
            );
            $ds = mysqli_fetch_array($ergebnis);
            $rate = round($ds[ 'points' ] / $ds[ 'votes' ]);
            safe_query(
                "UPDATE " . PREFIX . "plugins_articles SET rating='" . $rate . "' WHERE articleID='" . (int)$id."'"
            );
            $array[] = $id;
            $string_new = implode(":", $array);
            safe_query("UPDATE ".PREFIX."user SET articles='".$string_new."' WHERE userID='".(int)$userID."'");
    ...
    switch ($table) {
        case "articles":
            $table = "articles&action=content&articleID=" . $id;
            break;
        
    }

    header("Location: index.php?site=articles&action=content&articleID=" . $id."");
} else {
