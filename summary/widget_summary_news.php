...
#============================================================
    ...
    $template = $GLOBALS["_template"]->loadTemplate("sc_summary","headlines_head", $data_array, $plugin_path);
    echo $template;
...
$ergebnis = safe_query(
    "SELECT
        *
    FROM
        " . PREFIX . "plugins_news WHERE displayed = '1' ORDER BY
        date 
...
if (mysqli_num_rows($ergebnis)) {
    ...
    while ($ds = mysqli_fetch_array($ergebnis)) {
        ...
        $line1 = '<a class="p-1 badge bg-primary rounded-0" href="news_manager&action=news_contents&newsID==' . $ds[ 'newsID' ] . '" >READMORE</a>';
        $line = '' . $headline . '';
        ...
        $template = $GLOBALS["_template"]->loadTemplate("sc_summary","headlines_content", $data_array, $plugin_path);
        echo $template;
        $n++;
    }
    echo '</ul>';
...
#}
        $data_array = array();
        $template = $GLOBALS["_template"]->loadTemplate("sc_summary","headlines_foot", $data_array, $plugin_path);
        echo $template;
} else {
    
    $data_array = array();
    $data_array['$no_news_uploaded']=$plugin_language['no_news_uploaded'];
    $template = $GLOBALS["_template"]->loadTemplate("sc_summary","headlines_no_file", $data_array, $plugin_path);
    echo $template;
