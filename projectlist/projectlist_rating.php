...
if (isset($table) && isset($key)) {
    ...
    if (mysqli_num_rows($getlinks)) {
        ...
        if ($go === true) {
            safe_query(
                ...
                WHERE
                projectlistID = '" . (int)$id."'"
            );
            $ergebnis = safe_query(
                "SELECT votes, points FROM " . PREFIX . "plugins_projectlist WHERE projectlistID = '" . (int)$id."'"
            );
            $ds = mysqli_fetch_array($ergebnis);
            $rate = round($ds[ 'points' ] / $ds[ 'votes' ]);
            safe_query(
                "UPDATE " . PREFIX . "plugins_projectlist SET rating='" . $rate . "' WHERE projectlistID='" . (int)$id."'"
            );
            $array[] = $id;
            $string_new = implode(":", $array);
            safe_query("UPDATE ".PREFIX."user SET projectlist='".$string_new."' WHERE userID='".(int)$userID."'");
    ...
    switch ($table) {
        case "projectlist":
            $table = "projectlist&projectlistID=" . $id;
            break;
        
    }

    header("Location: index.php?site=projectlist&projectlistID=" . $id."");
} else {
