...
if ($action == "") {
    ...
    if (mysqli_num_rows($ergebnis)) {
        ...
        while ($ds = mysqli_fetch_array($ergebnis)) {
            ...
            } elseif (file_exists('includes/plugins/squads/images/games/' . $ds['game'] . '.avif')) {
                $gameicon = '../includes/plugins/squads/images/games/' . $ds['game'] . '.avif';
            } elseif (file_exists('includes/plugins/squads/images/games/' . $ds['game'] . '.webp')) {
                $gameicon = '../includes/plugins/squads/images/games/' . $ds['game'] . '.webp';
            } else {
                $gameicon = '../includes/plugins/squads/imagesimages/no-image.jpg';
            }
          ...
          data-toggle="tooltip" data-bs-html="true" title="' . $ds['opponent'] . '">';
            ...
            if (file_exists('includes/plugins/squads/images/squadicons/' . $ds['squad'] . '_small.jpg')) {
                ...
                $squadicon = '<img style="height: 50px" src="../includes/plugins/squads/images/squadicons/' . $ds['squad'] . '_small.avif" alt="" data-toggle="tooltip" data-bs-html="true" title="' . getsquadname($ds['squad']) . '">';
            } elseif (file_exists('includes/plugins/squads/images/squadicons/' . $ds['squad'] . '_small.webp')) {
                $squadicon = '<img style="height: 50px" src="../includes/plugins/squads/images/squadicons/' . $ds['squad'] . '_small.webp" alt="" data-toggle="tooltip" data-bs-html="true" title="' . getsquadname($ds['squad']) . '">';
            } else {
    ...
    if (file_exists('includes/plugins/squads/images/squadicons/' . $ds['squad'] . '_small.jpg')) {
        ...
        $squadicon = '<img style="height: 150px" src="../includes/plugins/squads/images/squadicons/' . $ds['squad'] . '_small.avif" alt="" data-toggle="tooltip" data-bs-html="true" title="' . getsquadname($ds['squad']) . '">';
    } elseif (file_exists('includes/plugins/squads/images/squadicons/' . $ds['squad'] . '_small.webp')) {
        $squadicon = '<img style="height: 150px" src="../includes/plugins/squads/images/squadicons/' . $ds['squad'] . '_small.webp" alt="" data-toggle="tooltip" data-bs-html="true" title="' . getsquadname($ds['squad']) . '">';
    } else {
    ...
    $scoreHome = unserialize($ds['homescore']);
    $scoreOpp = unserialize($ds['oppscore']);
    // Assicura che $scoreHome sia un array valido e filtra solo valori numerici
$scoreHome = is_array($scoreHome) ? array_filter($scoreHome, 'is_numeric') : [];

// Assicura che $scoreOpp sia un array valido e filtra solo valori numerici
$scoreOpp = is_array($scoreOpp) ? array_filter($scoreOpp, 'is_numeric') : [];
...
$oppscr = array_sum($scoreOpp);
    ...
    foreach ($theMaps as $map) {
        ...
        // Controlliamo se l'indice $d esiste negli array
        $homeScoreExists = isset($scoreHome[$d]) && is_numeric($scoreHome[$d]);
        $oppScoreExists = isset($scoreOpp[$d]) && is_numeric($scoreOpp[$d]);

        if ($homeScoreExists && $oppScoreExists) {
            ...
            // Se i punteggi non esistono, impostiamo un valore predefinito
            $score_1 = '<span class="text-muted">0</span>';
            $score_2 = '<span class="text-muted">0</span>';
        }
    ...
    while ($cwdata = mysqli_fetch_array($dp)) {
        ...
        $unserializeHome = @unserialize($ds['homescore']);
        $unserializeOpp = @unserialize($ds['oppscore']);

        // Controlla se la deserializzazione ha prodotto un array valido, altrimenti imposta un array vuoto
        $unserializeHome = is_array($unserializeHome) ? array_filter($unserializeHome, 'is_numeric') : [];
        $unserializeOpp = is_array($unserializeOpp) ? array_filter($unserializeOpp, 'is_numeric') : [];

        // Calcola il punteggio, assicurandoti che sia almeno 0
    ...
    if (mysqli_num_rows($squads)) {
        while ($squaddata = mysqli_fetch_array($squads)) {
            ...
            while ($squadcwdata = mysqli_fetch_array($squadcws)) {
                ...
                $unserializeSqHome = @unserialize($squadcwdata['homescore']);

                // Controlla se la deserializzazione ha prodotto un array valido, altrimenti imposta un array vuoto
                $unserializeSqHome = is_array($unserializeSqHome) ? array_filter($unserializeSqHome, 'is_numeric') : [];

                // Calcola il punteggio in modo sicuro
                $sqHomeScore = array_sum($unserializeSqHome);

                $totalHomeScoreSQ += $sqHomeScore;

                // total squad oppscore
                $unserializeSqOpp = @unserialize($squadcwdata['oppscore']);

                // Controlla se la deserializzazione ha prodotto un array valido, altrimenti imposta un array vuoto
                $unserializeSqOpp = is_array($unserializeSqOpp) ? array_filter($unserializeSqOpp, 'is_numeric') : [];

                // Calcola il punteggio in modo sicuro
                $sqOppScore = array_sum($unserializeSqOpp);
