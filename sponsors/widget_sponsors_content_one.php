...
if (mysqli_num_rows($ergebnis)) {
    ...
    <div class="row">
          <div class="sponsors_one">';
    $i = 1;
    while ($ds = mysqli_fetch_array($ergebnis)) {

        if (!empty($ds[ 'banner_small' ])) {
        $pic = '<img style="height: 80px" src="' . $filepath . $ds[ 'banner_small' ] . '" class="img-fluid" alt="Responsive image">';
        } else {
    ...
    box'.$ds['sponsorID'].'.addEventListener("touchend", function(e){
        ...
        $template = $GLOBALS["_template"]->loadTemplate("sponsors_one","content", $data_array, $plugin_path);
        echo $template;
        echo $script;
