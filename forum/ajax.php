...
$userID = $_GET['userID'];
	...
	if($loggedin){
		...
		if(!mysqli_num_rows(safe_query("SELECT * FROM ".PREFIX."plugins_forum_votes WHERE userID=".$userID." AND topicID=".$topic.""))){
			...
			if($_GET['v']==1){$value1=1;}else{$value1=0;}
			if($_GET['v']==2){$value2=1;}else{$value2=0;}
			if($_GET['v']==3){$value3=1;}else{$value3=0;}
			if($_GET['v']==4){$value4=1;}else{$value4=0;}
			if($_GET['v']==5){$value5=1;}else{$value5=0;}
			...
			echo 'Sie haben bereits für diese Umfrage gevotet!';
		}
