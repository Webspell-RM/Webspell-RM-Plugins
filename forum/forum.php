        ...
        while ($db = mysqli_fetch_array($boards)) {
            ...
            while ($lp = mysqli_fetch_assoc($q)) {
                if ($n == 1) {
                    ...
                    $topictitle = $lp['topic'];
                    if (mb_strlen($topictitle) > 122) {
                        $topictitle = mb_substr($topictitle, 0, 122);
                        $topictitle .= '...';
                    }
            ...
            if ($found) {
                $icon = '<button type="button" class="btn btn-primary btn-sm"><i class="bi bi-chat-dots"></i></button>';
            } else {
                $icon = '<button type="button" class="btn btn-secondary btn-sm"><i class="bi bi-chat"></i></button>';
            }

            // no entry mod - shows an text if no topic is open, also shows lastposter avatar from user if an topic is open //          
            if ($topictitle == '') {
                $noentry = '<td width="70%" class="text-end">' . $plugin_language['no_entry'] . '</td>
                    <td width="4%" class="text-end"></td>';
            } else {
                $noentry = '<td width="70%" class="text-end"><a href="' . $postlink . '" data-toggle="tooltip" title="' . $topictitletooltip . '">&nbsp;&nbsp;' . $topictitle . '</a> <br><small>' . $poster . '' . $member . ' - ' . $date . '</small> </td>

                    <td width="4%" class="text-end"><img class="avatar_small" src="../../images/avatars/' . $avatar . '" alt="' . $postername . '" title="' . $postername . '" /></td>';
            }
...
function showboard($board)
{
    ...
    $actions = '<a href="index.php?site=search" class="btn btn-secondary btn-sm"><i class="bi bi-search"></i> ' . $plugin_language['search'] . '</a>';
    if ($loggedin) {
        $mark = '<a href="index.php?site=forum&amp;board=' . $board . '&amp;action=markall">' .
            $plugin_language['mark_topics_read'] . '</a>';
        if ($writer) {
            $actions .= ' <a href="index.php?site=forum&amp;addtopic=true&amp;board=' . $board . '" class="btn btn-primary btn-sm"><i class="bi bi-chat-dots"></i> ' . $plugin_language['new_topic'] . '</a>';
        }
    } else {
        $mark = '';
    }
    ...
    if ($anztopics) {
        ...
        while ($dt = mysqli_fetch_array($topics)) {
            ...
            if ($dt['sticky']) {
                $onicon =
                    '<button type="button" class="btn btn-info btn-sm"><i class="bi bi-pin"></i></button>';
                $officon =
                    '<button type="button" class="btn btn-warning btn-sm"><i class="bi bi-pin"></i></button>';
                $onhoticon =
                    '<button type="button" class="btn btn-info btn-sm"><i class="bi bi-pin"></i></button>';
                $offhoticon =
                    '<button type="button" class="btn btn-warning btn-sm"><i class="bi bi-pin"></i></button>';
                ...
                $onicon =
                    '<button type="button" class="btn btn-primary btn-sm"><i class="bi bi-chat-dots"></i></button>';
                $officon =
                    '<button type="button" class="btn btn-secondary btn-sm"><i class="bi bi-chat"></i></button>';
                $onhoticon =
                    '<button type="button" class="btn btn-primary btn-sm"><i class="bi bi-chat-dots"></i></button>';
                $offhoticon =
                    '<button type="button" class="btn btn-secondary btn-sm"><i class="bi bi-chat"></i></button>';
            ...
            if ($dt['closed']) {
                $folder =
                    '<button type="button" class="btn btn-danger btn-sm"><i class="bi bi-lock"></i></button>';
            } elseif ($dt['moveID']) {
                $folder = '<button type="button" class="btn btn-secondary btn-sm"><i class="bi bi-arrow-right"></i></button>';
            } elseif ($userID) {
            ...
            $data_array['$lang_replies'] = $plugin_language['replies'];
        ...
        $data_array = array();
        $data_array['$no_topic'] = $plugin_language['no_topic'];
        $template = $GLOBALS["_template"]->loadTemplate("forum", "actions_head", $data_array, $plugin_path);
        echo $template;
    }
...
if (
    isset($_POST['submit']) || isset($_POST['movetopic']) || isset($_GET['addtopic'])
        ...
        if ($loggedin) {
            if (isset($_POST['preview'])) {
                ...
                if (isforumadmin($userID)) {
                    ...
                    $ergebnis = safe_query(
                        "SELECT * FROM " . PREFIX . "plugins_forum_ranks WHERE $posts >= postmin AND $posts <= postmax AND special='0'"
                    );
                    $ds = mysqli_fetch_array($ergebnis);
                    $usertype = $ds['rank'];
                ...
                $template = $GLOBALS["_template"]->loadTemplate("forum", "head_head", $data_array, $plugin_path);
                echo $template;
                echo'<div class="card-body forum-header"><h4 class="">Topic: ' . $topicname . '</h4></div>';
                ...
                $template = $GLOBALS["_template"]->loadTemplate("forum", "foot_foot", $data_array, $plugin_path);
                echo $template;
            } else {
                $topicname = "";
            }
            ...
            if (isforumadmin($userID) || ismoderator($userID, $board)) {
                if (isset($_POST['sticky'])) {
                    ...
                    $chk_sticky =
                      ...
                      <input class="form-check-input" type="checkbox" name="sticky" checked value="1"> 
                    ...
                    $chk_sticky =
                        '<div class="form-check form-switch">
                      <input class="form-check-input" type="checkbox" name="sticky" value="1"> 
                      <label class="form-check-label" for="flexSwitchCheckDefault">' . $plugin_language['make_sticky'] . '</label>
                    </div>';
                }
            } else {
