...
if (isset($_POST['newreply']) && !isset($_POST['preview'])) {
    ...
    if (
        !isforumposter($userID, $_POST['id']) && !isforumadmin($userID) && !ismoderator($userID, $_GET['board'])
    ) {
...
$topicactions = '<button id="printButton" class="btn btn-secondary btn-sm forum"><i class="bi bi-printer"></i></button>&nbsp;';

if ($loggedin && $writer) {
    $topicactions .=
        '<a href="index.php?site=forum&amp;addtopic=true&amp;action=newtopic&amp;board=' . $dt['boardID'] .
        '" class="btn btn-primary btn-sm forum hidden">' . $plugin_language['new_topic'] .
        '</a> <a href="index.php?site=forum_topic&amp;topic=' . $topic . '&amp;addreply=true&amp;page=' . $pages .
        '&amp;type=' . $type . '" class="btn btn-primary btn-sm forum"><i class="bi bi-share"></i> ' . $plugin_language['new_reply'] . '</a>';
}
if ($dt['closed']) {
        ...
        if (istopicpost($dt['topicID'], $id)) {
            ...
            if (mysqli_num_rows($notifyqry)) {
                $notify =
                  ...
                  <input class="form-check-input" type="checkbox" name="notify" checked value="1"> 
                  <label class="form-check-label" for="flexSwitchCheckDefault">' . $plugin_language['notify_reply'] . '</label>
                </div>';
            } else {
                $notify =
                    '<div class="form-check form-switch">
            ...
            if (isforumadmin($userID) || ismoderator($userID, $board)) {
                $chk_sticky =
                    '<div class="form-check form-switch">
          <input class="form-check-input" type="checkbox" name="sticky" value="1" ' . $_sticky . '> 
          <label class="form-check-label" for="flexSwitchCheckDefault">' . $plugin_language['make_sticky'] . '</label>
        </div>';
            } else {
                $chk_sticky = '';
            }
            ...
            if (mysqli_num_rows($notifyqry)) {
                $notify = '<div class="form-check form-switch">
                             <input class="form-check-input" type="checkbox" name="notify" checked value="1">
                             <label class="form-check-label" for="flexSwitchCheckDefault">' . $plugin_language['notify_reply'] . '</label></div>';
            } else {
                $notify = '<div class="form-check form-switch">
                             <input class="form-check-input" type="checkbox" name="notify" value="1">
                             <label class="form-check-label" for="flexSwitchCheckDefault">' . $plugin_language['notify_reply'] . '</label></div>';
            ...
            //STICKY
            if (isforumadmin($userID) || ismoderator($userID, $board)) {
                $chk_sticky = '<div class="form-check form-switch">
                             <input class="form-check-input" type="checkbox" name="sticky" value="1" ' . $_sticky . '>
                             <label class="form-check-label" for="flexSwitchCheckDefault">' . $plugin_language['make_sticky'] . '</label></div>';
            } else {
                $chk_sticky = '';
            }
    ...
    if ($loggedin && $writer) {
        if (isset($_POST['preview'])) {
            ...
            if ($getavatar = getavatar($userID)) {
                $avatar = '<img class="avatar" src="images/avatars/' . $getavatar . '" alt="">';
            } else {
                $avatar = '';
            }
            ...
            #$template = $GLOBALS["_template"]->loadTemplate("forum_topic", "head_head", $data_array, $plugin_path);
            #echo $template;
            echo'<div class="card-body forum-header"><h4 class="">' . $plugin_language['preview'] . '</h4></div>';
            ...
            $banned = '';
            if (isbanned($member)) {
                $banned = "<span class='text-danger'>" . $plugin_language['is_banned'] . "</span>";
            } else {
                $banned = '';
            }
        ...
        $template = $GLOBALS["_template"]->loadTemplate("forum_topic", "head_edit", $data_array, $plugin_path);
            echo $template;
        ...
        $template = $GLOBALS["_template"]->loadTemplate("forum_topic", "foot_foot", $data_array, $plugin_path);
        echo $template;
        $template = $GLOBALS["_template"]->loadTemplate("forum_topic", "foot_head", $data_array, $plugin_path);
        echo $template;
...
$ergebnis = safe_query("SELECT * FROM " . PREFIX . "plugins_forum_topics WHERE topicID=" . $dt['topicID'] . "");
$db = mysqli_fetch_array($ergebnis);

$data_array = array();

$data_array['$topicname'] = $topicname;
$data_array['$poster'] = getnickname($db['userID']);
$data_array['$date'] = getformatdate($db['date']);
$data_array['$time'] = getformattime($db['date']);
    ...
    if (mysqli_num_rows($ergebnis1) and $loggedin) {
        ...
        if (mysqli_num_rows($fd)) {
            while ($dd = mysqli_fetch_array($ergebnis1)) {
                ...
                $template = $GLOBALS["_template"]->loadTemplate("forum_topic", "vote_foot", $data_array, $plugin_path);
                echo $template;
            }
        } else {
            while ($dd = mysqli_fetch_array($ergebnis1)) {
                $title = $dd['title'];
                if (!empty($dd['value1'])) {
                    $value1 = '<table cellpadding="1" cellspacing="1" border="0" class="forum_poll_bg1"><tr><td align="center" width="5%"><input type="radio" style="border:0;" name="value" value="1" onclick="ForumVoteSave(\'' . $plugin_language['poll_really'] . '\',\'' . $_GET['topic'] . '\',\'1\',\'' . $loggedin . '\',\'' . $userID . '\');" /></td><td>' . $dd['value1'] . '</td></tr></table>';
                } else {
                    $value1 = '';
                }
                if (!empty($dd['value2'])) {
                    $value2 = '<table cellpadding="1" cellspacing="1" border="0" class="forum_poll_bg2"><tr><td align="center" width="5%"><input type="radio" style="border:0;" name="value" value="2" onclick="ForumVoteSave(\'' . $plugin_language['poll_really'] . '\',\'' . $_GET['topic'] . '\',\'2\',\'' . $loggedin . '\',\'' . $userID . '\');" /></td><td>' . $dd['value2'] . '</td></tr></table>';
                } else {
                    $value2 = '';
                }
                if (!empty($dd['value3'])) {
                    $value3 = '<table cellpadding="1" cellspacing="1" border="0" class="forum_poll_bg1"><tr><td align="center" width="5%"><input type="radio" style="border:0;" name="value" value="3" onclick="ForumVoteSave(\'' . $plugin_language['poll_really'] . '\',\'' . $_GET['topic'] . '\',\'3\',\'' . $loggedin . '\',\'' . $userID . '\');" /></td><td>' . $dd['value3'] . '</td></tr></table>';
                } else {
                    $value3 = '';
                }
                if (!empty($dd['value4'])) {
                    $value4 = '<table cellpadding="1" cellspacing="1" border="0" class="forum_poll_bg2"><tr><td align="center" width="5%"><input type="radio" style="border:0;" name="value" value="4" onclick="ForumVoteSave(\'' . $plugin_language['poll_really'] . '\',\'' . $_GET['topic'] . '\',\'4\',\'' . $loggedin . '\',\'' . $userID . '\');" /></td><td>' . $dd['value4'] . '</td></tr></table>';
                } else {
                    $value4 = '';
                }
                if (!empty($dd['value5'])) {
                    $value5 = '<table cellpadding="1" cellspacing="1" border="0" class="forum_poll_bg1"><tr><td align="center" width="5%"><input type="radio" style="border:0;" name="value" value="5" onclick="ForumVoteSave(\'' . $plugin_language['poll_really'] . '\',\'' . $_GET['topic'] . '\',\'5\',\'' . $loggedin . '\',\'' . $userID . '\');" /></td><td>' . $dd['value5'] . '</td></tr></table>';
                } else {
                    $value5 = '';
                }
...
while ($dr = mysqli_fetch_array($replys)) {
    ...
    /*if(deleteduser($dr['poster']) == '0'){
            ...
            $user_name = '<b>' .
                stripslashes(getnickname($dr['poster'])) . '</b>';    
        } else {
            $username = '<b>'.getnickname($dr['poster']).'</b>';
            $user_name = '<b>'.getnickname($dr['poster']).'</b>';
    ...
    if (@$dx['modulname'] != 'squads') {
        ...
        if (isclanmember($dr['poster'])) {
            $member = ' <i class="bi bi-person" style="color: #5cb85c"></i>';
            $me_mber = " <i class='bi bi-person text-success'></i>";
        } else {
            $member = '';
            $me_mber = '';
        }
    ...
    /*if ($getavatar = getavatar($dr['poster'])) {
            $avatar = '<img class="avatar_content" src="images/avatars/' . $getavatar . '" alt="'.$dr['poster'].'">';
        } else {
            $avatar = '';
        }*/
    ...
    if ($loggedin) {
        if ($userID !== $dr['poster']) {
            ...
            if (in_array($userID, $array)) {
                $danke = '<button id="klick" type="button" class="btn btn-like-danger btn-sm" id="ForumThankIt' . $dr['postID'] . '" onclick="ForumThankSave(\'' . $dr['postID'] . '\',\'0\',\'' . $loggedin . '\',\'' . $userID . '\');"> <i class="bi bi-hand-thumbs-down text-danger" style="font-size:16px" data-toggle="tooltip" data-placement="top" title="' . $plugin_language['disfeedback'] . '">' . $plugin_language['dislike'] . '</i></button>';
            } else {
                $danke = '<button id="klick" type="button" class="btn btn-like-success btn-sm" id="ForumThankIt' . $dr['postID'] . '" onclick="ForumThankSave(\'' . $dr['postID'] . '\',\'1\',\'' . $loggedin . '\',\'' . $userID . '\');"> <i class="bi bi-hand-thumbs-up text-success" style="font-size:16px" data-toggle="tooltip" data-placement="top" title="' . $plugin_language['feedback'] . '">' . $plugin_language['like'] . '</span></i></button>';
            }
        } else {
            $danke = '';
        }
    ...
    if ($dr['thank'] != '') {
        $thankhead .= '';
        $thankhead .= '<i class="bi bi-hand-thumbs-up text-success" style="font-size:13px">' . $plugin_language['like'] . ':</i> ';
        ...
        if (is_array($thank)) {
            ...
            foreach ($thank as $thanks) {
                if ($thanks != "") {
                    if ($n > 1) {
                        $getuser .= ', <a href="index.php?site=profile&amp;id=' . $thanks . '" target="_self" style="font-size:13px"> ' . getnickname($thanks) . '</a>';
                    }else{ $getuser = '<a href="index.php?site=profile&amp;id=' . $thanks . '" target="_self" style="font-size:13px"> ' . getnickname($thanks) . '</a>';
                    }
                    $n++;
                }
            }
    ...
    if (isonline($dr['poster']) == "offline") {
        ...
        $status_pic = "<span class='text-danger'>offline</span>";
    } else {
        $statuspic = '<span class="badge bg-success">online</span>';
        $status_pic = "<span class='text-success'>online</span>";
    }
    ...
    if (!$dt['closed']) {
        ...
        if ($loggedin) {
            $quote =
                ...
                '" data-toggle="tooltip" data-placement="top" title="' . $plugin_language['quote'] . '"><i class="bi bi-quote"></i> ' . $plugin_language['quote'] . '</a>
              ';
        }
    } else {
        $quote = '';
    ...
    $lastlogin = getlastlogin($dr['poster']);
    ...
    if (($userID == $dr['poster'] || isforumadmin($userID) || ismoderator($userID, $dt['boardID']))
        ...
        $actions = ' <a class="btn btn-warning text-dark btn-sm" href="index.php?site=forum_topic&amp;topic=' . $topic . '&amp;edit=true&amp;id=' .
            $dr['postID'] . '" data-toggle="tooltip" data-placement="top" title="' . $plugin_language['edit'] . '"><i class="bi bi-pencil"></i> ' . $plugin_language['edit'] . '</a> ';
    }
    if (isforumadmin($userID) || ismoderator($userID, $dt['boardID'])) {
        $actions .= '<input style="margin-top: 10px;" class="form-check-input" type="checkbox" name="postID[]" value="' . $dr['postID'] . '">';
    }

    #$user_date = '10.04.2024';
    $user_date = $lastlogin;

    if (deleteduser($dr['poster']) == '0') {
        $user_name = '<b>' .
            stripslashes(getnickname($dr['poster'])) . '</b>';
        $username = '<a href="index.php?site=profile&amp;id=' . $dr['poster'] . '" data-bs-toggle="popover"  data-bs-trigger="hover focus" data-bs-placement="top" data-bs-html="true" data-bs-content="' . $user_name . $me_mber . '<br>' . $usertype . ' ' . $special_rank . '<br>' . $plugin_language['posts'] . ': ' . $posts . ' | Onlinestatus: ' . $status_pic . '<br>Registriert am ' . $registered . ' | letzter Besuch: ' . $user_date . '"><b>' .
            stripslashes(getnickname($dr['poster'])) . '</b></a>';
    } else {
        $username = '<b>' . getnickname($dr['poster']) . '</b>';
        $user_name = '<b>' . getnickname($dr['poster']) . '</b>';
    ...
    if ($getavatar = getavatar($dr['poster'])) {
        $avatar = '<img 
            ...
            src="images/avatars/' . $getavatar . '" alt="' . $dr['poster'] . '" 
            data-bs-toggle="popover"  
            data-bs-trigger="hover focus" 
            data-bs-placement="top" 
            data-bs-html="true" 
            data-bs-content="' . $user_name . $me_mber . ' ' . $banned . '<br>' . $usertype . ' ' . $special_rank . '<br>' . $plugin_language['posts'] . ': ' . $posts . ' | Onlinestatus: ' . $status_pic . '<br>Registriert am ' . $registered . ' | letzter Besuch: ' . $user_date . '">';
    } else {
        $avatar = '';
    }
...
if (isforumadmin($userID) || ismoderator($userID, $dt['boardID'])) {
    ...
    $adminactions = '<div class="card">
            <div class="card-body">
                    <div class="row">
        <div class="col-md-7 d-flex justify-content-start align-self-center">
		<div class="form-check form-switch">
          <input class="form-check-input" type="checkbox" name="ALL" value="ALL" onclick="SelectAll(this.form);">
          <label class="form-check-label" for="flexSwitchCheckDefault">' . $plugin_language['select_all'] . '</label>
        </div></div>
